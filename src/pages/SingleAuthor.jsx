...
function SingleAuthor() {
  ...
  async function initalFetch(){
    try{
      const data = await GalleriesService.getAuthorGalleries(id);
      if(data){
        setGalleries(data.data);
        setMetadata(data.meta);
        setAuthor(data.data[0].user.first_name + ' ' + data.data[0].user.last_name);
      }
    }catch(err){
      console.log(err);
    }
  ...
  async function fetchMoreGalleries(){
    try{
      const data = await GalleriesService.getAuthorGalleries(id, currentPage);
      if(data){
  ...
  async function fetchFiltered(){
    if(filter.length > 0){
      const data = await GalleriesService.getAuthorGalleries(id, 1, filter);
